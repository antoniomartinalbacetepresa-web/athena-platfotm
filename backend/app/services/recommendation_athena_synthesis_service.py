...
class AthenaSynthesisResult:
    ...
    def to_api_dict(self) -> dict[str, Any]:
        return {
            ...
            **(
                {"newsSynthesisHash": self.news_synthesis_hash}
                if self.news_synthesis_hash is not None
                else {}
            ),
...
class RecommendationAthenaSynthesisService:
    ...
    The service does not execute a model. It verifies that a supplied model output
    is bound to the exact persisted Research Cycle and, when News evidence exists,
    to that cycle's canonical News synthesis. Every Radar evidence item must be
    referenced so a model cannot silently cherry-pick supportive evidence.
    """
        ...
        *,
        ...
        for candidate_index, candidate_raw in enumerate(candidates):
            ...
            for item_index, item_raw in enumerate(evidence):
                item = self._dict(
                    item_raw,
                    f"radar.candidates[{candidate_index}].evidence[{item_index}]",
                )
        ...
        if has_news:
            if not isinstance(news_synthesis_record, dict):
                raise ValueError(
                    "Un ciclo con evidencia News requiere su síntesis News canónica antes de ATHENA synthesis."
                )
            news_hash = self._sha256(
                news_synthesis_record.get("synthesis_hash"), "news_synthesis_record.synthesis_hash"
            )
            if self._sha256(
                news_synthesis_record.get("cycle_hash"), "news_synthesis_record.cycle_hash"
            ) != cycle_hash:
                raise ValueError("La síntesis News no pertenece al Research Cycle exacto.")
            if self._sha256(
                news_synthesis_record.get("radar_hash"), "news_synthesis_record.radar_hash"
            ) != radar_hash:
        ...
        expected_input_fingerprint = self.input_fingerprint(
        ...
        if generated_at < input_as_of:
            raise ValueError(
                "model_output.generated_at no puede preceder el inputAsOf del ciclo congelado."
            )
        ...
        uncertainties = self._unique_text_tuple(
            model_output.uncertainties,
            field="model_output.uncertainties",
            max_items=50,
            max_length=2000,
        )
        if not uncertainties:
            raise ValueError("model_output.uncertainties debe declarar al menos una incertidumbre.")
        referenced = self._unique_text_tuple(
            model_output.evidence_ids,
            field="model_output.evidence_ids",
            max_items=1000,
            max_length=256,
        )
        if set(referenced) != evidence_ids:
            raise ValueError(
                "ATHENA synthesis debe referenciar exactamente toda la evidencia Radar del ciclo."
            )
    ...
    def input_fingerprint(
        ...
        *,
        ...
        return self._canonical_hash(
            {
                "cycleHash": self._sha256(cycle_hash, "cycle_hash"),
                "radarHash": self._sha256(radar_hash, "radar_hash"),
                "newsSynthesisHash": (
                    self._sha256(news_synthesis_hash, "news_synthesis_hash")
                    if news_synthesis_hash is not None
                    else None
                ),
                "inputAsOf": self._aware_datetime(input_as_of, "input_as_of").isoformat(),
                "evidenceIds": sorted(self._text(item, "evidence_id") for item in evidence_ids),
                "coveredCategories": sorted(
                    self._text(item, "covered_category").lower() for item in covered_categories
                ),
            }
        )
    ...
    def _unique_text_tuple(
        ...
        *,
        ...
        if not isinstance(value, tuple):
            raise ValueError(f"{field} debe ser una tupla.")
        if len(value) > max_items:
            raise ValueError(f"{field} no puede superar {max_items} elementos.")
    ...
    def _assert_provider_allowed(self, provider: str) -> None:
        ...
        if any(token in compact for token in _FORBIDDEN_PROVIDER_TOKENS):
            raise ValueError("FMP/Financial Modeling Prep está prohibido en ATHENA synthesis.")
