    ...
    def to_api_dict(self) -> dict[str, Any]:
        return {
            ...
            "externalValidation": {
                ...
                "boundIdentityEvidenceFingerprint": self.identity_evidence_fingerprint
                if self.external_validation_passed
                else None,
...
class MarketWeightingReadinessService:
    ...
    constructor override that can bypass this gate in production.
    """
    ...
    def __init__(
        ...
        *,
        ...
        external_validation_repository: MarketWeightingExternalValidationRepository | None = None,
    ) -> None:
        ...
        self._external_validation_repository = (
            external_validation_repository
            if external_validation_repository is not None
            else MarketWeightingExternalValidationRepository(database=self._database)
        )
    ...
    def get_report(self, *, as_of: datetime | None = None) -> MarketWeightingReadinessReport:
        ...
        validation = self._external_validation_repository.get_latest_for_evidence(
        ...
        return MarketWeightingReadinessReport(
            ...
            external_validation_reference=(
                str(validation["reference"]) if validation is not None else None
            ),
            ...
            external_validation_fingerprint=(
                str(validation["validationFingerprint"])
                if validation is not None
                else None
            ),
        )

    def _identity_evidence_fingerprint(
        self,
        *,
        identity_market_cap_coverage: float,
        domicile_market_cap_coverage: float,
        ...
        canonical_listing_market_cap_count: int,
        median_fallback_market_cap_count: int,
    ) -> str:
        evidence = {
            "method": "canonical_domestic_listing_else_median_with_domicile",
            ...
            "regionMarketCapUsd": {
                key: float(region_market_cap_usd.get(key, 0.0))
                for key in ("america", "europe", "asia")
            },
