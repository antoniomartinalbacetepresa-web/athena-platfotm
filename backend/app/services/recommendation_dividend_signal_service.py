...
class RecommendationDividendSignal:
    ...
    def to_api_dict(self) -> dict[str, Any]:
        return {
            ...
            "reason": self.reason,
            "policy": {
                "temporal": "same_as_of_for_market_price_dividend_and_fundamental_knowledge",
                "yield": "trailing_dividend_cash_divided_by_explicit_pit_price",
                "currency": "yield_and_payout_blocked_when_currency_is_inconsistent",
                "totalReturn": "price_return_60d_plus_trailing_dividend_yield_diagnostic",
                "sustainability": "earnings_and_fcf_payout_only_with_positive_comparable_pit_per_share_denominators",
...
class RecommendationDividendSignalService:
    ...
    def __init__(self, *, database: AthenaDatabase | None = None, market_service: _MarketDiagnosticService | None = None, dividend_service: DividendAnalysisService | None = None, valuation_service: _ValuationDiagnosticService | None = None, free_cash_flow_service: _FreeCashFlowDiagnosticService | None = None) -> None:
        ...
        self._free_cash_flow_service = free_cash_flow_service or SecFreeCashFlowService()
    ...
    def evaluate(self, *, symbol: str, as_of: datetime) -> RecommendationDividendSignal:
        ...
        if self._parse_aware_datetime(market.get("asOf")) != as_of_utc:
        ...
        common = dict(symbol=normalized_symbol, instrument_id=instrument_id, as_of=as_of_utc.isoformat(), latest_price=latest_price, latest_price_observed_at=self._optional_text(market.get("latestObservedAt")), latest_price_retrieved_at=self._optional_text(market.get("latestRetrievedAt")), market_source_providers=source_providers, production_eligible=False, price_return_60d=self._optional_float(market.get("return60d")))
        ...
        total_return_60d = price_return_60d + dividend_yield if price_return_60d is not None and dividend_yield is not None else None
        valuation = self._valuation_payload(symbol=normalized_symbol, as_of=as_of_utc)
        earnings_payout_ratio = self._earnings_payout_ratio(dividend=dividend, valuation=valuation)
        fcf_payout_ratio = self._fcf_payout_ratio(dividend=dividend, valuation=valuation, as_of=as_of_utc)
    ...
    def _earnings_payout_ratio(self, *, dividend: dict[str, Any], valuation: dict[str, Any]) -> float | None:
        cash, currency = self._optional_float(dividend.get("annualizedCashPerShare")), self._optional_text(dividend.get("currency"))
        eps = valuation.get("annualDilutedEps")
        if cash is None or cash < 0 or currency is None or not isinstance(eps, dict):
            return None
        value, unit = self._optional_float(eps.get("value")), self._optional_text(eps.get("unit"))
        if value is None or value <= 0 or unit is None or unit.lower().replace(" ", "") not in {f"{currency.lower()}/share", f"{currency.lower()}/shares"}:
            return None
        return cash / value

    def _fcf_payout_ratio(self, *, dividend: dict[str, Any], valuation: dict[str, Any], as_of: datetime) -> float | None:
        cash, currency = self._optional_float(dividend.get("annualizedCashPerShare")), self._optional_text(dividend.get("currency"))
        entity_id = self._optional_text(valuation.get("entityId"))
        if cash is None or cash < 0 or currency is None or entity_id is None or not entity_id.startswith("sec-cik:"):
            return None
        diagnostic = self._free_cash_flow_service.evaluate(cik=entity_id.split(":", 1)[1], as_of=as_of)
        to_api_dict = getattr(diagnostic, "to_api_dict", None)
        if not callable(to_api_dict):
            raise RuntimeError("El diagnóstico FCF no respeta el contrato.")
        payload = to_api_dict()
        if not isinstance(payload, dict) or payload.get("productionEligible") is not False or self._parse_aware_datetime(payload.get("asOf")) != as_of:
            raise RuntimeError("El diagnóstico FCF devolvió un contrato PIT inválido.")
        fcf_per_share, unit = self._optional_float(payload.get("freeCashFlowPerShare")), self._optional_text(payload.get("unit"))
        if payload.get("status") != "diagnostic_ready" or fcf_per_share is None or fcf_per_share <= 0 or unit is None or unit.lower() != currency.lower():
            return None
    ...
    def _parse_aware_datetime(value: object) -> datetime:
        try:
            ...
            raise RuntimeError("El diagnóstico incluye asOf inválido.") from exc
        if parsed.tzinfo is None or parsed.utcoffset() is None:
            raise RuntimeError("El diagnóstico incluye asOf sin zona horaria.")
