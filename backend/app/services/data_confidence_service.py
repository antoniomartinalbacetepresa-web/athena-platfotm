...
class DataConfidenceService:
    """Build a transparent confidence score from multiple source observations."""
    ...
    def assess(
        ...
        *,
        ...
        if len(numeric_values) <= 1:
            ...
            agreement_score = max(0.0, 100.0 - min(dispersion_pct * 4.0, 100.0))

        diversity_bonus = min(len({item.provenance.source_id for item in items}) - 1, 3) * 3.0
        confidence_score = (
        ...
        return ConfidenceAssessment(
            ...
            dispersion_score=round(max(0.0, 100.0 - min(dispersion_pct, 100.0)), 2),
            sources_used=tuple(sorted({item.provenance.source_id for item in items})),
