    ...
    def to_api_dict(self) -> dict[str, Any]:
        return {
            ...
            "policy": {
                ...
                "structuredProvenance": "news_and_investors_evidence_require_provider_publisher_published_at_and_https_source_ref",
...
class RecommendationAthenaRadarService:
        ...
        *,
        ...
        for raw_candidate in candidates:
            ...
            for raw_evidence in raw_candidate.evidence:
                ...
                if category in {"news", "investors"}:
                    self._assert_structured_external_provenance(
                        category=category,
                        provider=provider,
                        publisher=publisher,
    ...
    def _assert_structured_external_provenance(
        self,
        *,
        category: str,
        provider: str | None,
        ...
        if provider is None:
            raise ValueError(
                f"La evidencia {category} requiere provider explícito para preservar provenance."
            )
        if publisher is None:
            raise ValueError(
                f"La evidencia {category} requiere publisher/primary source explícito para preservar provenance."
            )
        if published_at is None:
            raise ValueError(
                f"La evidencia {category} requiere published_at explícito para preservar temporalidad PIT."
            )
        parsed = urlparse(source_ref)
        if parsed.scheme.casefold() != "https" or not parsed.netloc:
            raise ValueError(
                f"La evidencia {category} requiere source_ref HTTPS absoluto y trazable."
            )
