...
class RecommendationSizeFactorExposureRepository:
    """Append-only tamper-evident storage for PIT size factor artifacts."""

    def __init__(self, database: AthenaDatabase | None = None, service: RecommendationSizeFactorExposureService | None = None) -> None:
        self._database = database or AthenaDatabase()
        self._service = service or RecommendationSizeFactorExposureService(database=self._database)
    ...
    def initialize(self) -> None:
        ...
        with self._database.connect() as connection:
            connection.executescript(
                """
                CREATE TABLE IF NOT EXISTS athena_size_factor_exposure_artifacts (
                ...
                CREATE INDEX IF NOT EXISTS idx_size_factor_exposure_instrument_asof
                ON athena_size_factor_exposure_artifacts(instrument_id, as_of, id);
                """
            )
    ...
    def append(self, *, artifact: dict[str, Any]) -> dict[str, Any]:
        ...
        with self._database.connect() as connection:
            existing = connection.execute(
                "SELECT * FROM athena_size_factor_exposure_artifacts WHERE factor_exposure_key = ?", (key,)
            ).fetchone()
            if existing is not None:
                record = self._row(existing)
                if record["artifact"] != validated:
                    raise ValueError("factorExposureKey de size ya existe con contenido distinto.")
                return self.validate_record(record)
            connection.execute(
                """INSERT INTO athena_size_factor_exposure_artifacts
                   (factor_exposure_key, instrument_id, as_of, artifact_json, created_at)
                   VALUES (?, ?, ?, ?, ?)""",
                (key, int(validated["instrumentId"]), str(validated["asOf"]), serialized, created_at),
            )
    ...
    def validate_record(self, record: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(artifact, dict):
            raise ValueError("Registro size factor exposure carece de artifact válido.")
        validated = self._service.validate_artifact(dict(artifact))
        expected = {
    ...
    def _row(row: Any) -> dict[str, Any]:
        if row is None:
            raise RuntimeError("No se pudo recuperar size factor exposure persistido.")
        try:
            artifact = json.loads(str(row["artifact_json"]))
        except (TypeError, ValueError, json.JSONDecodeError) as exc:
            raise ValueError("artifact_json de size factor exposure no es JSON válido.") from exc
        return {
            "id": int(row["id"]),
