...
class RecommendationPriceFactorExposureRepository:
    """Append-only tamper-evident storage for PIT price factor artifacts."""

    def __init__(
        self,
        database: AthenaDatabase | None = None,
        service: RecommendationPriceFactorExposureService | None = None,
    ) -> None:
        self._database = database or AthenaDatabase()
        self._service = service or RecommendationPriceFactorExposureService()
    ...
    def initialize(self) -> None:
        ...
        with self._database.connect() as connection:
            connection.executescript(
                """
                CREATE TABLE IF NOT EXISTS athena_price_factor_exposure_artifacts (
                ...
                CREATE INDEX IF NOT EXISTS idx_price_factor_exposure_instrument_asof
                ON athena_price_factor_exposure_artifacts(instrument_id, as_of, id);
                """
            )
    ...
    def append(self, *, artifact: dict[str, Any]) -> dict[str, Any]:
        ...
        with self._database.connect() as connection:
            existing = connection.execute(
                "SELECT * FROM athena_price_factor_exposure_artifacts WHERE factor_exposure_key = ?",
                (key,),
            ).fetchone()
            if existing is not None:
                record = self._row(existing)
                if record["artifact"] != validated:
                    raise ValueError("factorExposureKey de precio ya existe con contenido distinto.")
                return self.validate_record(record)
            connection.execute(
                """
                INSERT INTO athena_price_factor_exposure_artifacts (
    ...
    def validate_record(self, record: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(artifact, dict):
            raise ValueError("Registro price factor exposure carece de artifact válido.")
        validated = self._service.validate_artifact(dict(artifact))
        expected = {
    ...
    def _row(row: Any) -> dict[str, Any]:
        if row is None:
            raise RuntimeError("No se pudo recuperar price factor exposure persistido.")
        try:
            artifact = json.loads(str(row["artifact_json"]))
        except (TypeError, ValueError, json.JSONDecodeError) as exc:
            raise ValueError("artifact_json de price factor exposure no es JSON válido.") from exc
        return {
            "id": int(row["id"]),
