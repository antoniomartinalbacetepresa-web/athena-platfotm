...
def test_dividend_return_evidence_keeps_gross_net_costs_and_provenance(tmp_path) -> None:
    ...
    result = PortfolioDividendReturnEvidenceService(ledger).build(
        portfolio_id="personal", reporting_currency="EUR", period_start=START, period_end=END, as_of=AS_OF
    )
    ...
    assert result.provenance_refs == (
        "broker_statement:custody-fee-1", "broker_statement:withholding-tax-1", "issuer_filing:aapl-div-2026q1"
    )
...
def test_dividend_return_evidence_classifies_observed_quarterly_cadence(tmp_path) -> None:
    ...
    result = PortfolioDividendReturnEvidenceService(ledger).build(
        portfolio_id="personal", reporting_currency="EUR", period_start=datetime(2026, 1, 1, tzinfo=UTC), period_end=datetime(2026, 10, 1, tzinfo=UTC), as_of=cutoff
    )
...
def test_dividend_metrics_use_only_explicit_pit_fundamentals(tmp_path) -> None:
    ...
    fundamentals = DividendFundamentalEvidence(
        price=40.0, payout_ratio=0.55, fcf_payout_ratio=0.60,
        available_at=datetime(2026, 3, 20, tzinfo=UTC), source="issuer_filing", source_ref="aapl-2025-10k"
    )
    result = PortfolioDividendReturnEvidenceService(ledger).build(
        portfolio_id="personal", reporting_currency="EUR", period_start=datetime(2024, 12, 31, tzinfo=UTC), period_end=datetime(2026, 4, 1, tzinfo=UTC), as_of=cutoff, fundamentals=fundamentals
    )
...
def test_dividend_return_evidence_rejects_silent_fx(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="explicit FX conversion evidence"):
        PortfolioDividendReturnEvidenceService(ledger).build(
            portfolio_id="personal", reporting_currency="USD", period_start=START, period_end=END, as_of=AS_OF
        )
...
def test_dividend_return_evidence_excludes_information_unavailable_at_cutoff(tmp_path) -> None:
    ...
    ledger.append(
        ...
        item=PortfolioLedgerEventInput(
            portfolio_id="personal", event_type="cash_dividend", occurred_at=datetime(2026, 1, 20, tzinfo=UTC),
            available_at=datetime(2026, 2, 5, tzinfo=UTC), currency="EUR", amount=7.0, instrument_id="MSFT:XNAS",
            quantity=None, source="issuer_filing", source_ref="msft-late-dividend",
        ),
    )
    result = PortfolioDividendReturnEvidenceService(ledger).build(
        portfolio_id="personal", reporting_currency="EUR", period_start=START, period_end=END, as_of=AS_OF
