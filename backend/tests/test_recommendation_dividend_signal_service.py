...
def _seed(database: AthenaDatabase) -> int:
    ...
    CorporateActionRepository(database=database).save_many(instrument_id=instrument_id,source_provider="primary",retrieved_at=AS_OF,actions=[{"action_type":"dividend","effective_at":datetime(2026,2,1,tzinfo=timezone.utc),"cash_amount":0.5,"currency":"USD"},{"action_type":"dividend","effective_at":datetime(2026,5,1,tzinfo=timezone.utc),"cash_amount":0.5,"currency":"USD"}]); return instrument_id
...
def test_dividend_signal_uses_same_pit_price_and_cutoff(tmp_path: Path) -> None:
    ...
    assert result.status=="diagnostic_ready"; assert result.latest_price==20.0; assert result.dividend is not None; assert result.dividend["trailingCashPerShare"]==pytest.approx(1.0); assert result.dividend["trailingYield"]==pytest.approx(0.05); assert result.price_return_60d==pytest.approx(0.10); assert result.total_return_60d==pytest.approx(0.15); assert result.earnings_payout_ratio==pytest.approx(0.5); assert result.fcf_payout_ratio is None; assert result.dividend["knowledgeCutoff"]==AS_OF.isoformat(); assert result.production_eligible is False
...
def test_fcf_payout_uses_same_pit_cutoff_currency_and_positive_per_share_fcf(tmp_path: Path) -> None:
    ...
    assert result.fcf_payout_ratio==pytest.approx(0.25); assert result.production_eligible is False
