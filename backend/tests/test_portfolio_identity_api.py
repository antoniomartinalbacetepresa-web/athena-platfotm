...
class FakeIdentityResult:
    ...
    def to_api_dict(self):
        return {"databaseInstrumentId": 7, "canonicalInstrumentId": "AAPL@NASDAQ", "issuerId": "issuer:apple", "symbol": "AAPL", "exchange": "NASDAQ", "exchangeShortName": "NASDAQ", "currency": "USD", "sourceProvider": "yahoo_catalog", "retrievedAt": "2026-09-04T18:00:00+00:00", "resolutionMethod": "symbol_and_exchange_exact" if self._risk_ready else "unique_active_symbol", "exchangeVerified": self._risk_ready, "isRiskReady": self._risk_ready, "isWeightingReady": False, "recommendationPolicy": "no_advice", "productionEligible": False, "automaticTrading": False}
...
def test_portfolio_identity_api_preserves_passive_risk_contract(monkeypatch) -> None:
    ...
    monkeypatch.setattr("app.api.portfolio.PortfolioInstrumentIdentityService", FakeService)
    response = TestClient(app).get("/api/v1/portfolio/instrument-identity", params={"symbol": "AAPL", "exchange": "NASDAQ"})
...
def test_portfolio_identity_api_returns_diagnostic_unready_identity(monkeypatch) -> None:
    ...
    monkeypatch.setattr("app.api.portfolio.PortfolioInstrumentIdentityService", FakeService)
    response = TestClient(app).get("/api/v1/portfolio/instrument-identity", params={"symbol": "AAPL", "exchange": "NMS"})
    assert response.status_code == 200
    data = response.json()["data"]
...
def test_portfolio_identity_api_fails_closed_for_unresolvable_identity(monkeypatch) -> None:
    ...
    monkeypatch.setattr("app.api.portfolio.PortfolioInstrumentIdentityService", FakeService)
    response = TestClient(app).get("/api/v1/portfolio/instrument-identity", params={"symbol": "ABC", "exchange": "UNKNOWN"})
    assert response.status_code == 409
    assert response.json()["detail"] == "identidad ambigua"


def test_portfolio_identity_api_requires_symbol() -> None:
    response = TestClient(app).get("/api/v1/portfolio/instrument-identity")
