...
class _SnapshotRepository:
    ...
    def get_snapshot(self, snapshot_id):
        return {"id": 10, "benchmark_symbol": "SPY"}
...
def _outcome(*, realized_return=0.04, benchmark_return=0.01, excess_return=0.03):
    return {
        "horizon_days": 7,
...
def _service(outcome):
    return RecommendationShadowLiveCandidateEvaluationService(
        ...
        snapshot_repository=_SnapshotRepository([outcome]),
...
def test_consistent_return_identity_remains_shadow_only():
    ...
    assert (
        ...
        == "persisted_excess_return_must_equal_realized_return_minus_benchmark_return"
    )
