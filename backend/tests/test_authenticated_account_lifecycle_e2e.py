...
def _configure(monkeypatch, tmp_path) -> str:
    ...
    monkeypatch.setenv("ATHENA_PORTFOLIO_EVENT_LEDGER_PATH", str(tmp_path / "portfolio_event_ledger.jsonl"))
...
def test_authenticated_account_lifecycle_across_profile_and_portfolio(monkeypatch, tmp_path) -> None:
    """Owner lifecycle acceptance; local fixtures are not production evidence."""
    database_path = _configure(monkeypatch, tmp_path)
    with TestClient(app) as client:
        created = client.post("/api/v1/auth/register", json={"email": EMAIL, "password": ORIGINAL_PASSWORD, "displayName": "Lifecycle User"})
        ...
        profile = client.put("/api/v1/user/profile/preferences", headers=_headers(original_token), json={"riskTolerance": "balanced", "investmentHorizonYears": 12, "baseCurrency": "EUR", "objective": "long_term_growth"})
        assert profile.status_code == 200, profile.text
        assert profile.json()["policy"]["sensitivePreferencesEncrypted"] is True
        position = client.put("/api/v1/user/portfolio/positions", headers=_headers(original_token), json={"symbol": "AAPL", "exchange": "NASDAQ", "quantity": 3.5, "averagePurchasePrice": 150.25})
        ...
        changed = client.post("/api/v1/auth/change-password", headers=_headers(original_token), json={"currentPassword": ORIGINAL_PASSWORD, "newPassword": NEW_PASSWORD})
        assert changed.status_code == 204, changed.text
        assert client.get("/api/v1/auth/me", headers=_headers(original_token)).status_code == 401
        ...
        persisted_profile = client.get("/api/v1/user/profile/preferences", headers=_headers(rotated_token))
        assert persisted_profile.status_code == 200, persisted_profile.text
        preferences = persisted_profile.json()["data"]["preferences"]
        assert preferences["riskTolerance"] == "balanced"
        assert preferences["investmentHorizonYears"] == 12
        assert preferences["baseCurrency"] == "EUR"
        assert preferences["objective"] == "long_term_growth"
        persisted_portfolio = client.get("/api/v1/user/portfolio", headers=_headers(rotated_token))
        assert persisted_portfolio.status_code == 200, persisted_portfolio.text
        assert persisted_portfolio.json()["data"]["positionCount"] == 1
        persisted_position = persisted_portfolio.json()["data"]["positions"][0]
        ...
        clean_profile = client.get("/api/v1/user/profile/preferences", headers=_headers(returned_token))
        ...
        clean_portfolio = client.get("/api/v1/user/portfolio", headers=_headers(returned_token))
    ...
    with sqlite3.connect(database_path) as connection:
        profile_rows = connection.execute("SELECT COUNT(*) FROM athena_user_profile_preferences WHERE owner_user_id = ?", (original_user_id,)).fetchone()
        portfolio_rows = connection.execute("SELECT COUNT(*) FROM athena_user_portfolio_positions WHERE owner_user_id = ?", (original_user_id,)).fetchone()
        original_account = connection.execute("SELECT email, display_name, is_active FROM athena_user_accounts WHERE id = ?", (original_user_id,)).fetchone()
    assert profile_rows is not None and int(profile_rows[0]) == 0
    assert portfolio_rows is not None and int(portfolio_rows[0]) == 0
    assert original_account == (f"closed-{original_user_id}@account.invalid", None, 0)
