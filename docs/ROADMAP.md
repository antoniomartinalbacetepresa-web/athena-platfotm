...
| Personalization | 76% | Owner-scoped risk tolerance, investment horizon, base currency, objective, experience level, liquidity need, maximum drawdown tolerance and available capital have validated AES-256-GCM persistence, authenticated Flutter transport and Profile UI load/save/reload/delete. Presentation-only personalization cannot influence recommendation scoring, canonical weighting, automatic learning promotion or trading. Versioned key rotation/migration is transactional and fail-closed. The Profile personalization projection now has CI-validated semantic heading, explicit no-authority/no-order boundary, accessible loading/error announcements and regressions that keep sensitive capital/currency out of presentation metadata. Remaining: deployment key-management procedure, broader cross-surface E2E/UX acceptance and production operational evidence. |
...
| Testing & CI | 98% | Backend pytest, Flutter analyze/test and a release Web build now feed a single fail-closed `Release validation gate`; the release bundle is retained as a CI artifact. Security, auth, Profile, Portfolio, market-history, corporate-action, recovery, provenance, navigation and News synthesis acceptance have dedicated regression coverage. Exact-SHA CI `d6a89e6bc80fb7e6aeb55cd67b8d795f764e500f` passed backend tests, Flutter analyze/tests, release Web build and the release gate in run #3218. Remaining: make checks mandatory through branch protection/rulesets and complete the remaining cross-surface E2E/security/recovery-delivery acceptance. |
...
- Exact-SHA CI for `d6a89e6bc80fb7e6aeb55cd67b8d795f764e500f` completed successfully in run #3218: backend tests, Flutter analyze/tests, the release Web build and the fail-closed `Release validation gate` are green.
- The Profile personalization projection now exposes a semantic heading, explicit presentation-only/no-recommendation/no-weighting/no-learning/no-order boundary, and accessible loading/error announcements. Regression coverage verifies those semantics while continuing to exclude sensitive capital and currency values from presentation metadata.
- This validated acceptance increment justifies moving whole-product Personalization from 75% to 76%. The analytical Flutter/UI score and overall whole-product estimate remain unchanged because this is a focused acceptance increment, not enough evidence for another aggregate rounding step.
