...
| News | 75% | Verified backend feed remains separate from the canonical latest News synthesis. Flutter consumes the canonical synthesis fail-closed and presents model summary, importance, estimated impact magnitude/direction, confidence, publisher/model identity and HTTPS provenance. Acceptance coverage verifies the canonical endpoint, rejects FMP and authority escalation, and proves 404/503/retry cannot fabricate or retain stale synthesis. Remaining: broader navigation/accessibility/E2E acceptance and production evidence for external synthesis execution/corroboration; model output remains presentation-only and cannot influence recommendations, scoring or trading. |
| Portfolio | 88% | Portfolio UI plus valuation, concentration, correlations, attribution/reconciliation and accounting infrastructure exist. Authenticated personal positions are isolated server-side by JWT-derived ownership. Declared average purchase price is persisted with AES-256-GCM and synchronized only through authenticated transport. The owner-scoped append-only Event Ledger is exposed through an authenticated read-only history API with PIT `asOf` filtering. Flutter consumes that same ledger through a typed fail-closed model. Cross-surface regressions prove owner changes, revoked sessions and confirmed account closure cannot reuse Profile-derived Portfolio capital/currency. Remaining: broader Portfolio E2E/UX acceptance and production key-management evidence. |
...
| Testing & CI | 98% | Backend pytest, Flutter analyze/test and a release Web build now feed a single fail-closed `Release validation gate`; the release bundle is retained as a CI artifact. Security, auth, Profile, Portfolio, market-history, corporate-action, recovery, provenance, navigation and News synthesis acceptance have dedicated regression coverage. Exact-SHA CI `33c0d24cf7774c76a87a5a1c5a6ba4bb118dc740` passed the backend, Flutter, release-build and release-gate jobs. Remaining: make checks mandatory through branch protection/rulesets and complete the remaining cross-surface E2E/security/recovery-delivery acceptance. |
...
- Exact-SHA CI for `33c0d24cf7774c76a87a5a1c5a6ba4bb118dc740` completed successfully: backend tests, Flutter analyze/tests, the release Web build, release artifact upload and the fail-closed `Release validation gate` are green.
- The account-lifecycle cross-surface boundary now verifies both outcomes: confirmed backend closure clears the persisted authenticated authority and prevents Portfolio from reusing Profile-derived owner state; rejected closure preserves the same owner's session and state instead of falsely logging the user out.
- Dashboard primary navigation has wide/compact acceptance coverage, application routes preserve their named `RouteSettings`, and Market refresh integration is fail-closed against stale snapshots.
- These changes justify moving Profile/authentication from 91% to 92% and Testing/CI from 97% to 98%. Overall whole-product completion remains approximately 84%; the increments are not large enough to justify rounding the product upward.
