...
class _AuthenticatedPortfolioPageState extends State<AuthenticatedPortfolioPage> {
  ...
  Widget _buildAuthoritativeAuthenticatedPortfolio() {
    ...
    return Scaffold(body: SafeArea(child: SingleChildScrollView(padding: const EdgeInsets.all(20), child: Center(child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 1200), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [IconButton(tooltip: 'Volver', onPressed: () => Navigator.of(context).maybePop(), icon: const Icon(Icons.arrow_back_rounded)), const SizedBox(width: 8), const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Semantics(header: true, child: Text('MI CARTERA', style: TextStyle(fontSize: 30, fontWeight: FontWeight.bold))), Text('Posiciones personales protegidas por tu cuenta ATHENA'), SizedBox(height: 4), Text('Vista informativa: no ejecuta órdenes ni modifica recomendaciones o pesos automáticamente.', style: TextStyle(fontSize: 12))])), IconButton(key: const Key('portfolio-authenticated-history'), tooltip: 'Historial de cuenta', onPressed: controller.sessionRejected ? null : _openAuthenticatedHistory, icon: const Icon(Icons.history))]),
