...
import '../../di/market_dependencies.dart';
...
class _GlobalMarketPanelState extends State<GlobalMarketPanel> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        Expanded(child: _marketContextView(globalContext)),
      ],
    );
  }
  ...
  Widget _buildGlobalMarketView(GlobalMarketContext globalContext) {
    final sentimentColor = _sentimentColor(globalContext.sentiment);
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Text(
                    _sentimentLabel(globalContext.sentiment),
                    style: TextStyle(
              ...
              children: [
                Expanded(
                  child: _secondaryMetric(
                    label: 'BENCHMARKS',
                    value: _totalAssets(globalContext).toString(),
                    compact: compact,
                  ),
                ),
                Expanded(
                  child: _secondaryMetric(
                    label: 'SUBIENDO',
                    value: '${globalContext.advancingPercentage.toStringAsFixed(1)}%',
                    compact: compact,
                  ),
                ),
                Expanded(
                  child: _secondaryMetric(
                    label: 'BAJANDO',
                    value: '${globalContext.decliningPercentage.toStringAsFixed(1)}%',
                    compact: compact,
                  ),
                ),
              ],
            ...
            Row(
              children: [
                Expanded(
                  child: _secondaryMetric(
                    label: 'ACTUALIZADO',
                    value: _formatUpdatedAt(globalContext.updatedAt),
                    compact: compact,
                  ),
                ),
                Expanded(
                  child: _secondaryMetric(
                    ...
                    value: '${(globalContext.weightConfidence * 100).toStringAsFixed(0)}%',
                    compact: compact,
                  ),
                ),
              ],
  ...
  String _coverageLabel(GlobalMarketContext context) {
    ...
    if (context.hasRealMarketUniverse && !status.isWeightingReady) {
      return 'UNIVERSO REAL PERSISTIDO · ${status.globallyUsableCount} ACTIVOS · PESOS EN CALIBRACIÓN';
    }
    if (context.hasRealMarketUniverse) {
      return 'UNIVERSO REAL PERSISTIDO · ${status.globallyUsableCount} ACTIVOS PONDERABLES · DIRECCIÓN POR BENCHMARKS';
    }
    return 'COBERTURA PILOTO · DIRECCIÓN POR BENCHMARKS · PESOS CON FALLBACK';
  }
  ...
  Widget _regionalRow({
    ...
    final color = _sentimentColor(context.sentiment);
    return Container(
      width: double.infinity,
      padding: EdgeInsets.symmetric(
        horizontal: 10,
  ...
  int _totalAssets(GlobalMarketContext globalContext) {
  ...
  Color _sentimentColor(String sentiment) {
    switch (sentiment.toLowerCase()) {
      case 'positive':
        return AthenaColors.success;
      case 'negative':
        return AthenaColors.danger;
      default:
