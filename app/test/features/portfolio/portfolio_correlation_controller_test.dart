...
void main() {
  test('loads verified correlation and clears for a single position', () async {
    ...
    final controller = PortfolioCorrelationController(
      service: PortfolioCorrelationEvidenceService(
        loadPair: ({required leftInstrumentId, required rightInstrumentId, required sourceProvider, required knowledgeCutoff, observedFrom, observedTo}) async =>
            pair(leftInstrumentId, rightInstrumentId, cutoff, 0.4),
      ),
    );

    await controller.load(positions: [position('AAA', 1), position('BBB', 2)], knowledgeCutoff: cutoff);
    expect(controller.snapshot?.meanCorrelation, 0.4);
    expect(controller.error, isNull);

    await controller.load(positions: [position('AAA', 1)], knowledgeCutoff: cutoff);
  ...
  test('fails closed without exposing partial snapshot', () async {
    ...
    final controller = PortfolioCorrelationController(
      service: PortfolioCorrelationEvidenceService(
        loadPair: ({required leftInstrumentId, required rightInstrumentId, required sourceProvider, required knowledgeCutoff, observedFrom, observedTo}) async =>
            throw StateError('history missing'),
      ),
    );
  ...
  test('older async result cannot overwrite a newer request', () async {
    ...
    final service = PortfolioCorrelationEvidenceService(
      loadPair: ({required leftInstrumentId, required rightInstrumentId, required sourceProvider, required knowledgeCutoff, observedFrom, observedTo}) {
        calls++;
    ...
    final oldLoad = controller.load(positions: [position('AAA', 1), position('BBB', 2)], knowledgeCutoff: cutoff);
    await controller.load(positions: [position('CCC', 3), position('DDD', 4)], knowledgeCutoff: cutoff);
